...
<div id="{{ $comment->id }}" class="my-1 small card-subtitle text-muted pt-2 pb-3">
    <div class="d-flex bd-highlight">
        <div class="align-self-top">
            ...
            <img class="rounded-circle" src="{{ $comment->user->getImageThumbnail($comment->user->id) }}" width="50px" height="50px" alt="Card image">
            </a>
        ...
        <div class="align-self-center bd-highlight flex-grow-1">
            ...
            <div class="pl-3 py-2 card-subtitle text-muted text-dark d-block" style="font-size: 16px">
                <div class="d-flex bd-highlight">
                    <div class="align-self-center mr-auto">
                        ...
                        @if($comment->reply_id)
                            ...
                            @foreach($comment->replies as $comment_reply)
                                <a class="small text-muted text-black-50" href="{{ route('profile.show', $comment_reply->user->id) }}">
                                    @if ($comment_reply->user->first_name && $comment_reply->user->last_name)
                                        {{ $comment_reply->user->first_name }}
                                    @else
                    ...
                    <div class="comment-header-tools align-self-center text-muted text-black-50">
                        ...
                        <form method="POST" action="{{ route('profile.wall.messages.comments.delete', [$comment->user, $comment->wall_message_id, $comment]) }}">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-sm py-0 px-0 bg-transparent fa fa-close text-light text-muted"></button>
                        </form>
                    </div>
                </div>
                ...
                <div class="comment-footer-tools">
                    <a href="#replay" id="{{ $comment->id }}" class="replay" title="@if ($comment->user->first_name && $comment->user->last_name) {{ $comment->user->first_name }} @else {{ $comment->user->name }} @endif" comment-id="{{ $comment->id }}">{{ __('comments.Replay') }}</a>
                </div>
            </div>
        </div>
    </div>
